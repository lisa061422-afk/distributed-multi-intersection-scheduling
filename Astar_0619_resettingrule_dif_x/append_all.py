"""
append_all.py — appends Test 2 (50 trials) and Test 3 (20 trials) to existing PPT.
Fixed: max 13 rows per slide to prevent vertical overflow.
"""
...
def title_bar(slide, title, subtitle=None):
    ...
    box(slide, 0.3, 0.10, 12, 0.6, title, font_size=28, bold=True,
        fg=C_WHITE, align=PP_ALIGN.LEFT)
    if subtitle:
        box(slide, 0.3, 0.65, 12, 0.4, subtitle, font_size=14,
            fg=RGBColor(0xD0,0xE8,0xFF), align=PP_ALIGN.LEFT)

def divider(title, subtitle, note=None):
    sl = prs.slides.add_slide(BLANK); bg(sl)
    rect(sl, 0, 2.8, 13.33, 0.08, C_BLUE)
    box(sl, 1, 1.5, 11.3, 1.0, title,
        font_size=40, bold=True, fg=C_BLUE, align=PP_ALIGN.CENTER)
    box(sl, 1, 2.6, 11.3, 0.5, subtitle,
        font_size=16, fg=C_MGRAY, align=PP_ALIGN.CENTER)
    if note:
        box(sl, 1, 3.3, 11.3, 0.5, note,
            font_size=14, fg=C_YELLOW, align=PP_ALIGN.CENTER)

# ── table helper ─────────────────────────────────────────────────────────────
HDR   = ["#","N","Routes","d1 (s)","Cost ON","Cost OFF","Match",
         "Nodes ON","Nodes OFF","Lvs ON","Lvs OFF","t_ON(s)","t_OFF(s)","Tout"]
COL_W = [0.30, 0.25, 1.18, 2.05, 0.70, 0.70, 0.54,
         0.70, 0.73, 0.54, 0.54, 0.64, 0.70, 0.46]
# sum = 9.03", x0=0.25 → right edge 9.28" — well within 13.33"
# rh=0.44", y0=1.25": 13 data rows → 1.25 + 14*0.44 = 7.41" < 7.5" ✓

def table_slide(title, subtitle, data):
    sl = prs.slides.add_slide(BLANK); bg(sl)
    title_bar(sl, title, subtitle)
    x0, y0, rh = 0.25, 1.25, 0.44
    xc = x0
    for h, cw in zip(HDR, COL_W):
        rect(sl, xc, y0, cw-0.02, rh-0.03, C_BLUE)
        box(sl, xc+0.02, y0+0.05, cw-0.06, rh-0.10, h,
    ...
    for ri, row in enumerate(data):
        ...
        m = row[6]
        if   m == "NO★": bg_row = RGBColor(0xFC,0xE8,0xE6)
        elif m == "N/A":  bg_row = RGBColor(0xFE,0xF5,0xE4)
        xc = x0
...
def summary_slide(title, subtitle, stats, perf, bullets, footer):
    ...
    for i, (val, label, col) in enumerate(stats):
        ...
        box(sl, x, 2.2, 3.6, 0.7, label, font_size=13,
            fg=C_WHITE, align=PP_ALIGN.CENTER)
    box(sl, 0.5, 3.15, 12.3, 0.4, footer,
...
# ══════════════════════════════════════════════════════════════════════════════
# SIMULATION TEST 2  (50 trials, seed=123)
# ══════════════════════════════════════════════════════════════════════════════
t2 = [
...
divider("Simulation Test 2",
        "50 random trials  ·  N_sys ∈ {3–6}  ·  routes ∈ {1–12}  ·  d1 ∈ U[0,3 s]  ·  seed=123",
        None)

ROWS_PER_PAGE = 13
for i in range(0, len(t2), ROWS_PER_PAGE):
    chunk = t2[i:i+ROWS_PER_PAGE]
    a, b = chunk[0][0], chunk[-1][0]
    table_slide(f"Simulation Test 2 — Trials {a}–{b}",
                "50 random cases · WeakRule ON vs OFF · seed=123", chunk)

summary_slide(
    "Simulation Test 2 — Summary Statistics", "50 trials · seed=123",
    [("36 / 50","Cost identical\n(ON = OFF)", C_GREEN),
     ("2 / 50", "Cost differs\n(Trials 3 & 23 — ON cheaper)", C_YELLOW),
     ("12 / 50","N/A — OFF timed out\n(ON completed in 10/12)", C_RED)],
    [("Avg time ON","2.30 s",C_GREEN),("Avg time OFF","9.07 s",C_RED),
     ("Speedup","~4×",C_BLUE),("ON timeouts","2/50",C_YELLOW),("OFF timeouts","12/50",C_RED)],
    ...
    "WeakRule ON is NEVER worse than OFF — in both cost-differ cases, ON finds the better solution"
)

# ══════════════════════════════════════════════════════════════════════════════
# SIMULATION TEST 3  (20 trials, seed=456, N=7-10)
# ══════════════════════════════════════════════════════════════════════════════
t3 = [
...
for i in range(0, len(t3), 10):
    chunk = t3[i:i+10]
    a, b = chunk[0][0], chunk[-1][0]
    table_slide(f"Simulation Test 3 — Trials {a}–{b}",
                "N_sys ∈ {7–10} · stress test · seed=456", chunk)

summary_slide(
    "Simulation Test 3 — Summary & Scalability Analysis", "N_sys ∈ {7–10} · 20 trials · seed=456",
    [("5 / 20", "ON finds solution\n(OFF all timeout)", C_GREEN),
     ("0 / 20", "Cost match\n(OFF never completes)", C_MGRAY),
     ("20 / 20","All N/A\n(OFF 100% timeout)", C_RED)],
    [("Avg ON","23.83 s",C_YELLOW),("Avg OFF","30.01 s",C_RED),
     ("ON solves","5/20",C_GREEN),("OFF solves","0/20",C_RED),("Best ON","0.12 s",C_GREEN)],
    ["• Trial 16 (N=9): ON solves in 0.19 s / 298 nodes — sparse conflict, WeakRule prunes aggressively",
     "• Trial 20 (N=8): ON solves in 0.12 s / 228 nodes — fastest in batch",
     "• Trials 8/15/19: ON finds solution in 3–15 s; OFF still times out",
     "• N=10 (Trials 3, 10): both methods fail — search space exceeds 30 s budget",
     "• Practical range: ON handles N≤7 reliably; N=8–9 depends on contention; N≥10 needs heuristics"],
    "WeakRule ON still finds solutions where OFF completely fails — advantage grows with N"
)

# ── Combined footer slide ─────────────────────────────────────────────────────
sl = prs.slides.add_slide(BLANK); bg(sl)
rect(sl, 0, 0, 13.33, 1.1, C_BLUE)
box(sl, 0.3, 0.1, 12, 0.9, "Overall Conclusion — 90 Random Trials",
    font_size=28, bold=True, fg=C_WHITE, align=PP_ALIGN.LEFT)
data = [("Test 1","20 trials, N=3–6, seed=42", "15","1","4","0.54 s","8.47 s"),
        ("Test 2","50 trials, N=3–6, seed=123","36","2","12","2.30 s","9.07 s"),
        ("Test 3","20 trials, N=7–10, seed=456","0*","0","20","23.83 s","30.01 s")]
hdrs = ["Test","Config","Cost Match","ON Better","N/A","Avg t_ON","Avg t_OFF"]
cws  = [0.8, 2.8, 1.2, 1.2, 0.9, 1.2, 1.2]
x0 = 0.5; y0 = 1.25; rh = 0.52
xc = x0
for h, cw in zip(hdrs, cws):
    rect(sl, xc, y0, cw-0.03, rh-0.04, C_BLUE)
    box(sl, xc+0.02, y0+0.1, cw-0.07, rh-0.15, h, font_size=11,
        bold=True, fg=C_WHITE, align=PP_ALIGN.CENTER)
    xc += cw
for ri, row in enumerate(data):
    y = y0 + (ri+1)*rh
    bg_c = RGBColor(0xF4,0xF6,0xF8) if ri%2==0 else RGBColor(0xFF,0xFF,0xFF)
    xc = x0
    for ci, (v, cw) in enumerate(zip(row, cws)):
        rect(sl, xc, y, cw-0.03, rh-0.04, bg_c)
        fc = C_GREEN if ci==2 and v!="0*" else (C_YELLOW if ci==3 else C_LGRAY)
        box(sl, xc+0.02, y+0.1, cw-0.06, rh-0.15, v, font_size=11,
            fg=fc, align=PP_ALIGN.CENTER)
        xc += cw

box(sl, 0.5, 3.15, 12.3, 0.4,
    "* Test 3: OFF never completes — ON finds 5/20 solutions. No case where ON cost > OFF cost across all 90 trials.",
    font_size=11, fg=C_MGRAY, align=PP_ALIGN.LEFT)
pts = [
    (C_GREEN,  "Optimality: WeakRule ON is NEVER suboptimal — 0 counterexamples in 90 trials"),
    (C_YELLOW, "Superiority: 3 cases where ON finds strictly better solutions than OFF (mechanism confirmed)"),
    (C_BLUE,   "Efficiency: ON is 4–16× faster than OFF on N=3–6; only viable option for N≥7"),
    (C_RED,    "Limitation: N≥8 high-contention cases may still timeout — heuristic pruning recommended"),
]
for i, (col, txt) in enumerate(pts):
    rect(sl, 0.5, 3.75+i*0.72, 0.1, 0.5, col)
    box(sl, 0.7, 3.8+i*0.72, 12.1, 0.55, txt, font_size=13, fg=C_LGRAY)
