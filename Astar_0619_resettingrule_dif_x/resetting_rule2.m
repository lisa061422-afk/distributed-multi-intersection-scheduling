function [ra_temp, V_temp,x] = resetting_rule2(ra,ra_temp,V_temp,V_c,tw,da,NODES,l,x,ni2,cfg)

N = cfg.N;  Map = cfg.Map;  C = cfg.C;
tw1 = tw1_basedon_V_temp(V_temp,ra,da,tw,N); % next sig m based on each branch selection
for n = 1:N
    if any(ra(:,n) > 0.00001) && all(V_temp(n, :) == 0) % vehicle n's task is interrupted
        ...
        elseif s_interuptted == 2 %第二个subtask be interruptted
            a1 = Map(1,n); % which space a
            t1 = tw1 - C(1,n); %如果sub1能在tw1完成，那么sub1的开始时刻  
            ...
            t_avail = check_x(t_avail, 1, a1, x, n, C, N);
            % for p = 1:N
            %     if p == n || isempty(x{p})
            %         continue;
            %     end
            ...
            ra_temp(1,n) = t_avail + C(1,n) - tw1; % won't < 0 
            ra_temp(2:end,n) = C(2:end,n); 
        %------------------sub 3 interrupted-------------------------------
        elseif s_interuptted == 3 %第三个subtask be interruptted
              b1 = Map(1,n); % sub1占用哪个 space b1
              a2 = Map(2,n); % sub2占用哪个resource, space a2
              t22 = tw1 - C(2,n); 
              ...
              %----------------check x to ensure t_avail--------------------
              ta2_avail = check_x(ta2_avail, 2, a2, x, n, C, N);
              % for p = 1:N
              %   if p == n || isempty(x{p})
              %       continue;
              %   end
            ...
            %-------------------------------------------------------------
              if tb1 <= t21 + 1e-5 % (15) holds
                  ...
                  ra_temp(2,n) = ta2_avail + C(2,n) - tw1; % won't < 0
                  ra_temp(3,n) = C(3,n); 
