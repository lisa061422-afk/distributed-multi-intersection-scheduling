function branches = resetting_rule2(ra,ra_temp,V_temp,V_c,tw,da,NODES,l,x,ni2,cfg,pair_lock,reset_since)
...
if nargin < 12 || isempty(pair_lock),   pair_lock   = zeros(N, N); end
if nargin < 13 || isempty(reset_since), reset_since = zeros(1, N); end
...
for n = interrupted
    ...
    for bi = 1:numel(branches)
        ...
        x_b{n}{ni2(n)} = {};   % discard stale reservations — refreshed each interrupt
        ...
        if s_int == 1
            ra_b(:,n) = C(:,n);
            pl_b      = relock_Vtemp(pl_b, V_b, n, Map(1,n));
        ...
        elseif s_int == 2
            ...
            for vi = 1:numel(vars)
                ...
                if ~isempty(displaced)
                    % Displace branch: n takes a1, displaced system fully reset
                    np = displaced;
                    ra_vi(:,np)        = C(:,np);
                    x_vi{np}{ni2(np)}  = {};
                    rs_vi(np)          = tw;
                    ...
                    x_vi{n}{ni2(n)} = [x_vi{n}{ni2(n)}; {tw1-C(1,n), tw1, 1, a1}];
                    ra_vi(1,n)      = 0;
                    ra_vi(2:end,n)  = C(2:end,n);
                else
                    % Wait branch: compute t_avail accounting for reset_since
                    [~,tb1] = check_resc_occupation(NODES,valid_a1,a1,l,V_vi,n,tw1,tw,rs_vi);
                    [tb1, blkr] = check_x(tb1, 1, a1, x_vi, n, C, N);
                    if blkr > 0 && blkr ~= n
                    ...
                    if tb1 <= t1_start + 1e-5
                        ...
                        x_vi{n}{ni2(n)} = [x_vi{n}{ni2(n)}; {tb1, tb1+C(1,n), 1, a1}];
                        ra_vi(1,n) = tb1 + C(1,n) - tw1;
                    end
                    ra_vi(2:end,n) = C(2:end,n);
                end
        ...
        elseif s_int == 3
            ...
            for vi2 = 1:numel(vars_a2)
                ...
                if ~isempty(disp2)
                    ...
                    ta2_avail = tw1;   % a2 cleared, n starts sub2 at tw1
                else
                    [~,ta2] = check_resc_occupation(NODES,valid_a2,a2,l,V_v2,n,tw1,tw,rs_v2);
                    ta2_avail = max(ta2, t22);
                    [ta2_avail, blkr2] = check_x(ta2_avail, 2, a2, x_v2, n, C, N);
                    if blkr2>0 && blkr2~=n
                ...
                for vi1 = 1:numel(vars_b1)
                    ...
                    if ~isempty(disp1)
                        ...
                        if tb1 <= t21 + 1e-5
                            ...
                            x_v1{n}{ni2(n)} = [x_v1{n}{ni2(n)}; {tb1_2, min(tb1_2+C(1,n),tw1), 1, b1}];
                            x_v1{n}{ni2(n)} = [x_v1{n}{ni2(n)}; {min(tb1_2+C(1,n),tw1), tw1, 2, a2}];
                            ra_v1(1,n) = max(0, tb1_2 + C(1,n) - tw1);
                            ra_v1(2,n) = min(tb1_2 + C(1,n) - tw1 + C(2,n), C(2,n));
                            ra_v1(3,n) = C(3,n);
                        end
                    end
...
% Returns: cell of {V_variant, displaced_system_or_empty}
%--------------------------------------------------------------------------
function variants = space_variants(V_temp, m, n, pair_lock, reset_since)
    ...
    % Wait variant: n' keeps m (always when lock unknown or np won)
    if lock_val == 0 || lock_val == np
        variants{end+1} = {V_temp, []};
    end

    % Displace variant: only when n has explicitly established priority
    if lock_val == n
        V_B      = V_temp;
        V_B(np, m) = 0;
