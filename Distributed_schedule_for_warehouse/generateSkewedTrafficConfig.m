function [config, vehicleList, stats] = generateSkewedTrafficConfig(numVehicles, varargin)
%GENERATESKEWEDTRAFFICCONFIG
% Like generateBalancedTrafficConfig, but deliberately concentrates
% scheduling load on one "hot" intersection while keeping the others light.
%
% Usage:
%   [config, vehicleList, stats] = generateSkewedTrafficConfig(N, ...
%       'HotIntersection', 1, 'HotFraction', 0.65, 'Seed', 42);
%
% Key parameters:
%   'HotIntersection'  (1–4, default 1) : which intersection gets heavy load
%   'HotFraction'      (0–1, default 0.65): target fraction of all vehicles
%                      that pass through the hot intersection
%   'HotWeight'        (default 4.0) : extra emphasis on hitting hot target
%                      (higher → more extreme skew)
%
% All other parameters are identical to generateBalancedTrafficConfig.

    p = inputParser;
    addRequired(p, 'numVehicles', @(x)isnumeric(x) && isscalar(x) && x>=1);

    addParameter(p, 'HotIntersection',  1,    @(x)isnumeric(x) && isscalar(x) && ismember(x,1:4));
    addParameter(p, 'HotFraction',      0.65, @(x)isnumeric(x) && isscalar(x) && x>0 && x<1);
    addParameter(p, 'HotWeight',        4.0,  @(x)isnumeric(x) && isscalar(x) && x>0);

    addParameter(p, 'MaxPerEntrance',       ceil(numVehicles/8)+1, @(x)isnumeric(x) && isscalar(x) && x>=1);
    addParameter(p, 'EntrancePenalty',      0.20, @(x)isnumeric(x) && isscalar(x) && x>=0);
    addParameter(p, 'StagePenalty',         0.50, @(x)isnumeric(x) && isscalar(x) && x>=0);
    addParameter(p, 'SameODPenalty',        0.15, @(x)isnumeric(x) && isscalar(x) && x>=0);
    addParameter(p, 'LongRoutePenalty',     0.20, @(x)isnumeric(x) && isscalar(x) && x>=0);
    ...
    hotInt     = p.Results.HotIntersection;
    hotFrac    = p.Results.HotFraction;
    hotWeight  = p.Results.HotWeight;
    ...
    % ── target loads ──────────────────────────────────────────────────────
    % hot intersection: hotFrac * N vehicles pass through it
    % the other 3 intersections share the remaining load evenly
    hotTarget  = hotFrac * numVehicles;
    coldTarget = (1 - hotFrac) * numVehicles / 3;
    intTarget  = coldTarget * ones(1, 4);
    intTarget(hotInt) = hotTarget;

    % per-intersection scoring weight: hot intersection gets extra emphasis
    intWeight  = ones(1, 4);
    intWeight(hotInt) = hotWeight;
    ...
    for k = 1:numVehicles
        ...
        % bias pool toward routes through hotInt when load is still below target
        hotDeficit = hotTarget - totalIntLoad(hotInt);
        if hotDeficit > 0 && rand < 0.6 && ~isempty(longCands)
        ...
        nPool = numel(pool);
        if nPool == 0
            error('Candidate pool empty. Try increasing MaxPerEntrance.');
        end
        ...
        for c = 1:nPool
            ...
            % 1) weighted deviation from skewed targets (core difference)
            targetScore = sum(intWeight .* (newTotalIntLoad - intTarget).^2);

            % 2) stage-wise balance among cold intersections only
            coldInts = setdiff(1:4, hotInt);
            stageScore = 0;
            for s = 1:3
                col = newStageLoad(coldInts, s).';
                if any(col)
                    stageScore = stageScore + sum((col - mean(col)).^2);
                end
            end

            % 3) entrance balance (mild)
            entranceScore = sum((newEntranceLoad - mean(newEntranceLoad)).^2);

            % 4) same OD repetition penalty
            repScore = countSameOD(vehicleList, e, x);

            % 5) mild long-route penalty
    ...
    stats.hotIntersection  = hotInt;
    stats.hotFraction      = hotFrac;

    fprintf('\nGenerated %d vehicles (SKEWED: hot=I%d, target=%.0f%%).\n', ...
        numVehicles, hotInt, hotFrac*100);
...
% ── local helpers (identical to generateBalancedTrafficConfig) ────────────
