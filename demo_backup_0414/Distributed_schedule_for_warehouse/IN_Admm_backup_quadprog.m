...
% quadprog: min 0.5*x'Hx + f'x  =>  H = 4*(rho1+rho2)*I, f = B_n per system
A_coef = 4*(rho1 + rho2);
...
for i = 1:length(LEAF)
    ...
    for ni = 1:nv
        ...
        C_vec(n) = C_n;
        % B_n = derivative of objective at x_n=0 (linear coefficient)
    ...
    if exitflag <= 0
        ...
        x_opt = zeros(N, 1);
        for ni = 1:nv
            x_opt(valid_systems(ni)) = x_vec(ni);
        end
        y_opt = x_opt + C_vec;
        ...
        if any(isnan(x_opt(valid_systems))) || any(isnan(y_opt(valid_systems)))
            ...
            % Evaluate true objective at solution (needed for correct leaf comparison)
            cost = 0;
            for ni2 = 1:nv
                n = valid_systems(ni2);
