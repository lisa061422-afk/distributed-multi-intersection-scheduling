function [x_prev, y_prev, x_prev_bar, y_prev_bar] = ...
    initEarliestXYPrev_fromChain(alpha_tilde, pathInfo_agent_chain, pathInfo_c, Dt, N, perturbScale)
% perturbScale (optional): max random delay added per vehicle (uniform [0, scale]).
%   0 or omitted → deterministic earliest-time init (original behaviour).
...
for n = 1:N
    ...
    % per-vehicle random offset — uniform [0, perturbScale]
