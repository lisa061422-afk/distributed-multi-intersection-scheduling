"""
Seed sweep for 20-vehicle 2x2 warehouse runs.

Status taxonomy:
  density-reject  config rejected — some intersection would exceed --max_per_int
  iter1-slow      first ADMM iter exceeded --first_iter_timeout (decision tree
                  exploded for some agent — abandoned, NO valid result)
  no-conv         ran to max_iter, residuals never dropped to tol (ADMM diverged)
  slow-conv       converged but ADMM elapsed > --total_budget (valid result,
                  too slow for demo)
  ok              converged within budget (valid result, fast)

Run from repo root:
    python python_port/sweep_seeds_20r.py
"""
...
def run_one(seed, first_iter_timeout, max_iter, max_per_int):
    cmd = [
    ...
    info = {'seed': seed, 'wall': wall, 'rc': proc.returncode,
            'iters': None, 'admm_elapsed': None, 'delay': None,
            'first_iter': None, 'reason': ''}
    ...
    for line in out.splitlines():
        ...
        if s.startswith('iterations'):
            try:
                info['iters'] = int(s.split('=')[1].split('/')[0].strip())
            except Exception:
                pass
        elif s.startswith('elapsed'):
            try:
                info['admm_elapsed'] = float(s.split('=')[1].replace('s','').strip())
            except Exception:
                pass
        elif s.startswith('delay cost'):
            try:
                info['delay'] = float(s.split('=')[1].strip())
            except Exception:
                pass
...
def main() -> int:
    ...
    ap.add_argument('--total_budget', type=float, default=30.0,
                    help='Above this, converged runs are tagged slow-conv')
    ap.add_argument('--max_per_int', type=int, default=8,
                    help='Hard cap on vehicles per intersection')
    ap.add_argument('--max_iter', type=int, default=300)
    args = ap.parse_args()

    print(f'Sweep: N=20  2x2 warehouse  Dt=3  T_val=2  max_per_int={args.max_per_int}')
    ...
    for seed in args.seeds:
        info = run_one(seed, args.first_iter_timeout, args.max_iter, args.max_per_int)
        st = info['status']
