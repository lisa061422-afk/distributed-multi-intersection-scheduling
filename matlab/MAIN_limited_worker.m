...
if skip_normal_run
    ...
    checkpointFile = fullfile(caseDir, 'admm_checkpoint.mat');
    [x_prev, y_prev, LocalTreeCache, residual_r, residual_s, delay_costs, k, T_ADMM_TOTAL] = ...
        run_admm_limited(const, agent_participation, checkpointFile);
...
if enable_priority_sweep && ~isempty(priority_robots)
    ...
    for prio_n = priority_robots
        ...
        checkpointFile_p = fullfile(caseDir_p, 'admm_checkpoint.mat');
        [xp, yp, LCp, rr_p, rs_p, dc_p, kp, Tp] = ...
            run_admm_limited(const_p, agent_participation, checkpointFile_p);
...
function [x_prev, y_prev, LocalTreeCache, residual_r, residual_s, ...
          delay_costs, k, T_ADMM] = run_admm_limited(const, agent_participation, checkpointFile)
