...
% Add this module and the shared scheduler code using repo-relative paths.
paths = setup_five_intersection();
if isempty(which('sdpvar'))
    error('YALMIP is required. Add its folder to the MATLAB path before running.');
end
if isempty(which('gurobi'))
    error('Gurobi is required and must be configured for MATLAB/YALMIP.');
end

% ===================== Mode switch =====================
configMode = 'manual';   % 'manual' | 'random'
...
useParallel = false;      % safe default; set true when Parallel Toolbox is available
...
if useParallel
    if license('test','Distrib_Computing_Toolbox')
        ...
        NUM_WORKERS = min(NUM_AGENTS - 1, clusterMax);
        p = gcp('nocreate');
        ...
        warning('Parallel Computing Toolbox is unavailable; using sequential execution.');
        useParallel = false;
    end
end
...
for iS = 1:numel(seedList)
    ...
    if strcmp(configMode, 'manual')
        ...
        config_raw = {
            ...
            struct('entrance', 4, 'exits', [8 2]),   % I3-W → I5-N, I1-W
        };
    ...
    if skip_normal_run
        ...
        %% ── ADMM run ─────────────────────────────────────────────────────
        checkpointFile = fullfile(caseDir, 'admm_checkpoint_5int.mat');
        [x_prev, y_prev, LocalTreeCache, residual_r, residual_s, delay_costs, k, T_ADMM] = ...
            run_admm_5int(const, agent_participation, NUM_AGENTS, checkpointFile);
        ...
        export_panel_grid_png(panelHandles, [localBase '.png']);
        ...
        fig_fcfs = figure('Color','w','Position',[50 50 2200 1080], ...
            'Scrollable','off');
        panelHandles_fcfs = gobjects(1,5);
        for pi = 1:5
            agent_i = int_order(pi);
        ...
        export_panel_grid_png(panelHandles_fcfs, [fcfsBase '.png']);
    ...
    if enable_priority_sweep && ~isempty(priority_robots)
        ...
        for prio_n = priority_robots
            ...
            checkpointFile_p = fullfile(caseDir_p, 'admm_checkpoint_5int.mat');
            [xp, yp, LCp, rr_p, rs_p, dc_p, kp, Tp] = ...
                run_admm_5int(const_p, const_p.agent_participation, NUM_AGENTS, checkpointFile_p);
            ...
            export_panel_grid_png(panelHandles_p, [localBase_p '.png']);
...
function [x_prev, y_prev, LocalTreeCache, residual_r, residual_s, ...
          delay_costs, k, T_ADMM] = run_admm_5int(const, agent_participation, NUM_AGENTS, checkpointFile)
...
for k = 1:max_iter
    ...
    if mod(k,10)==0
        save(checkpointFile,'x_prev','y_prev','LocalTreeCache', ...
